...
>
> 起票し直す Issue は **self-contained** にする。元 Issue を本文にリンク・引用しない
> （エージェントが辿って未検証の内容を読む恐れがあるため）。トレーサビリティは
> **生成された PR ↔ 元 Issue** をリンクする形で残す（PR は人がレビューするので安全）。
> propose / apply のプロンプトにも「参照先を辿るな」というガードを入れてある。
